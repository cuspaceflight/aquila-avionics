...
int8_t SI4463_PowerUp(const si4463_t *si4463) {
	...
	uint8_t answer[SI4463_CMD_BUF_LEN];
	memset(answer, 0x00, SI4463_CMD_BUF_LEN);
	uint8_t cmdChain[7] = { SI4463_CMD_POWER_UP, 0x00, 0x00, (RADIO_CONFIGURATION_DATA_RADIO_XO_FREQ
			& 0xFF000000) >> 24, (RADIO_CONFIGURATION_DATA_RADIO_XO_FREQ & 0x00FF0000) >> 16,
...
int8_t SI4463_Get_PH_Status(si4463_t *si4463) {
	...
	uint8_t answer[SI4463_CMD_BUF_LEN];
	memset(answer, 0x00, SI4463_CMD_BUF_LEN);
	uint8_t cmdChain[1] = { SI4463_CMD_GET_PH_STATUS };
	uint8_t phPend, phStatus;

	result += SI4463_SendCommand(si4463, cmdChain, 1);
	result += SI4463_ReadCommandBuffer(si4463, answer, 3);
	...
	if (result == SI4463_OK && answer[1] == SI4463_BYTE_CTS) {
		...
		/* PH pending Status */
		si4463->phStatus.filter_match_pend = (phPend >> 7) & 0x01;
		si4463->phStatus.filter_miss_pend = (phPend >> 6) & 0x01;
		si4463->phStatus.packet_sent_pend = (phPend >> 5) & 0x01;
		si4463->phStatus.packet_rx_pend = (phPend >> 4) & 0x01;
		si4463->phStatus.crc_error_pend = (phPend >> 3) & 0x01;
		si4463->phStatus.tx_fifo_almost_empty_pend = (phPend >> 1) & 0x01;
		si4463->phStatus.rx_fifo_almost_full_pend = phPend & 0x01;

		/* PH status*/
		si4463->phStatus.filter_match = (phStatus >> 7) & 0x01;
		si4463->phStatus.filter_miss = (phStatus >> 6) & 0x01;
		si4463->phStatus.packet_sent = (phStatus >> 5) & 0x01;
		si4463->phStatus.packet_rx = (phStatus >> 4) & 0x01;
		si4463->phStatus.crc_error = (phStatus >> 3) & 0x01;
		si4463->phStatus.tx_fifo_almost_empty = (phStatus >> 1) & 0x01;
		si4463->phStatus.rx_fifo_almost_full = phStatus & 0x01;
	}
...
int8_t SI4463_StartRx(const si4463_t *si4463, const uint16_t len, const bool goToRxAfterTimeout,
		const bool goToRxAfterValid, const bool goToRxAfterInvalid) {
	int8_t result = SI4463_OK;
	...
	RADIO_CONFIGURATION_DATA_CHANNEL_NUMBER, 0x00, (len & 0xFF00) >> 8, len & 0xFF,
			stateAfterTimeout, stateAfterValid, stateAfterInvalid };
...
int8_t SI4463_StartTx(const si4463_t *si4463, const uint16_t len, const bool goToRxAfterTx) {
	...
	uint8_t cmdChain[5] =
			{ SI4463_CMD_START_TX,
			RADIO_CONFIGURATION_DATA_CHANNEL_NUMBER, stateAfterTx, ((len & 0xFF00) >> 8) & 0x1F, len
					& 0xFF };
...
int8_t SI4463_ClearSharedFifo(const si4463_t *si4463) {
	int8_t result = SI4463_OK;
	uint8_t cmdChain[2] = { SI4463_CMD_FIFO_INFO, 0x01 | 0x02 };
...
int8_t SI4463_GetProperty(const si4463_t *si4463, const uint8_t group, const uint8_t numProps,
		const uint8_t startProp, uint8_t *data) {
	int8_t result = SI4463_OK;
	uint8_t cmdChain[4];
	memset(cmdChain, 0, 4);
	uint8_t answer[SI4463_CMD_BUF_LEN];
...
int8_t SI4463_SetProperty(const si4463_t *si4463, const uint8_t group, const uint8_t numProps,
		const uint8_t startProp, const uint8_t *data) {
	int8_t result = SI4463_OK;
